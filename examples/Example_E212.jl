...
Res = collect(linspace(10000,130000,13))
Ms = collect(linspace(0,.3,5))
r_over_R = 0.75
c_over_r = 0.3
TSR = 8.0
...
airfoil_file = joinpath(folder,"data","E212_xy.txt")
headerlines = 0
open(airfoil_file,"r") do f
    ...
    for (i,line) in enumerate(eachline(f))
        if i>headerlines
            append!(x,parse(split(chomp(line))[1]))
            append!(y,parse(split(chomp(line))[2]))
        else
        end
    end
end
...
JLD.save("$(fileLoc)/Data/af_prop_E212.jld", "NDtable", NDtable)
...
fileLoc,_ = splitdir(@__FILE__)
NDtable = JLD.load("$(fileLoc)/Data/af_prop_E212.jld")
